...
class ItemController extends GetxController implements GetxService {
  ...
  List<ItemModel?> moduleWiseItems = [];
  Map<String, int> moduleWiseIDs = {};
  bool modulewiseLoading = true;
  ...
  void applyFilters({bool isPopular = false, bool isSpecial = false}) {
    if (isPopular) {
      getPopularItemList(
          notify: true, offset: '1', dataSource: DataSourceEnum.client);
    } else if (isSpecial) {
  ...
  Future<void> getModuleWise(
      {required String offset,
      required int moduleId,
      DataSourceEnum dataSource = DataSourceEnum.client,
      bool notify = false,
      bool firstTimeCategoryLoad = false}) async {
    ItemModel? itemModel;
    if (offset == '1') {
      _offsetList = [];
      _offset = 1;
    ...
    if (!_offsetList.contains(offset)) {
      ...
      itemModel = await itemServiceInterface.getModuleWiseItems(
      ...
      log('fetched $itemModel');

      // itemModel!.offset = moduleId;
      moduleWiseIDs
          .addAll({itemModel?.items?.first.moduleType ?? "": moduleId});

      moduleWiseItems.add(itemModel);

      // log('after update $moduleId ${itemModel?.items?.first.name}');
      modulewiseLoading = false;
      update();
    } else {
      if (isLoading) {
        _isLoading = false;
  ...
  Future<void> getPreDiscountedItemList(
      {required String offset,
    ...
    if (!_offsetList.contains(offset)) {
      ...
      ItemModel? itemModel = await itemServiceInterface.getModuleWiseItems(
      ...
      if (dataSource == DataSourceEnum.local) {
        getDiscountedItemList(
            notify: notify, dataSource: DataSourceEnum.client, offset: '1');
      }
    } else {
      if (isLoading) {
        _isLoading = false;
  ...
  void _preparePopularItems(
      ItemModel? itemModel, String offset, bool firstTimeCategoryLoad) {
    if (itemModel != null) {
      if (offset == '1') {
        _popularItemList = [];
  ...
  Future<void> getItemDetails(
      {required int itemId, CartModel? cart, Item? item}) async {
    _item = null;
    if (item?.name != null) {
      _item = item;
    } else {
  ...
  void navigateToItemPage(Item? item, BuildContext context,
      {bool inStore = false, bool isCampaign = false, int? moduleId}) async {
    log('module id $moduleId');
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (moduleId != null) {
      prefs.setInt('mid', moduleId);
    }
    ...
    getItemDetails(itemId: item!.id!).then((value) {
      if (((_item!.foodVariations != null && _item!.foodVariations!.isEmpty) &&
        ...
        double discountPrice = PriceConverter.convertWithDiscount(
            price, discount, _item!.discountType)!;
