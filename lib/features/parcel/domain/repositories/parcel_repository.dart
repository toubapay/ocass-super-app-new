...
class ParcelRepository implements ParcelRepositoryInterface {
  ...
  Future<List<ParcelCategoryModel>?> _getParcelCategory() async {
    ...
    log('response of parcel category: ${response.body}');
    if (response.statusCode == 200) {
      parcelCategoryList = [];
  ...
  Future<ParcelCancellationReasonsModel?> getParcelCancellationReasons(
      {required bool isBeforePickup}) async {
    ...
    if (response.statusCode == 200) {
      cancellationReasonsModel =
          ParcelCancellationReasonsModel.fromJson(response.body);
    }
