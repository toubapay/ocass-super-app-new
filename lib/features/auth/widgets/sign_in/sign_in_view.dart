...
import 'package:demandium/common/models/response_model.dart'
    as demandium_response;
...
class _SignInViewState extends State<SignInView> {
  ...
  void initState() {
    ...
    _countryDialCode = authController.getUserCountryCode().isNotEmpty
        ...
        : CountryCode.fromCountryCode(splashController.configModel!.country!)
            .dialCode;
    ...
    switch (centralizeLogin) {
      ...
      case CentralizeLoginType.social:
        return SocialLoginWidget(
            onlySocialLogin: true, backFromThis: widget.backFromThis);
      ...
      if (!phoneValid.isValid) {
        ...
        authController
          ...
          if (response.isSuccess) {
            log('trying to send otp for verification screen from main');
            ServiceAuthController serviceAuthController =
                Get.find<ServiceAuthController>();
            demandium_response.ResponseModel responseModel =
                await serviceAuthController
                    .sendOtpForVerificationScreenFromMain(
                        identity: numberWithCountryCode,
                        identityType: 'phone',
                        checkUser: 0);
            log('response model from send otp for verification screen from main: ${responseModel.content?['otp']}');

            if (responseModel.isSuccess ?? false) {
              _processOtpSuccessSetup(
                  response, authController, phone, countryDialCode,
                  serviceOtp: responseModel.content?['otp']?.toString());
            } else {
              showCustomSnackBar(responseModel.message ?? 'otp_send_failed'.tr);
            }
          } else {
  ...
  void _processOtpSuccessSetup(ResponseModel response,
      AuthController authController, String phone, String countryDialCode,
      {String? serviceOtp}) async {
    log('inserting otp in process otp success setup: ${serviceOtp ?? 'no otp'}');
    if (authController.isActiveRememberMe) {
