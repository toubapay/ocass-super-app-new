...
class _NewUserSetupScreenState extends State<NewUserSetupScreen> {
  ...
  Widget build(BuildContext context) {
    log('building new user setup screen with phone: ${widget.phone} and email: ${widget.email}');
    return Scaffold(
                      ...
                      return CustomButton(
                        ...
                        onPressed: () async {
                          log(widget.phone == null
                              ? 'No phone number provided, proceeding with email: ${widget.email}'
                              : 'Phone number provided: ${widget.phone}, proceeding with phone verification');
                          if (_formKeyInfo!.currentState!.validate()) {
                            if (widget.phone == null || widget.phone!.isEmpty) {
                              String numberWithCountryCode = _countryDialCode! +
  ...
  void _updatePersonalInfo(
    ...
    authController
      ...
      if (response.isSuccess) {
        ...
        log('verifying code for service auth controller ');
        await serviceAuthController.updateNewUserProfileAndLoginFromMain(
          phone: (widget.phone != null && widget.phone!.isNotEmpty)
              ? widget.phone
              : numberWithCountryCode,
          email: widget.email ?? _emailController.text.trim(),
