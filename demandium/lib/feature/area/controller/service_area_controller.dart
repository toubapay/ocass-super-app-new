...
class ServiceAreaController extends GetxController implements GetxService {
    ...
    DataSyncHelper.fetchAndSyncData(
      ...
      onResponse: (data, source) {
        ...
        data['content']['data'].forEach(
          (zone) => _zoneList!.add(ZoneModel.fromJson(zone)),
        );
  ...
  Future<void> setMarker(
    ...
    for (int index = 0; index < zoneList.length; index++) {
      ...
      for (
        int subIndex = 0;
        subIndex < zoneList[index].formattedCoordinates!.length;
        subIndex++
      ) {
        zoneLatLongList.add(
          LatLng(
            zoneList[index].formattedCoordinates![subIndex].latitude!,
            zoneList[index].formattedCoordinates![subIndex].longitude!,
          ),
        );
      }
      markerList.add(
        Marker(
          ...
          icon: GetPlatform.isWeb || GetPlatform.isIOS
              ? BitmapDescriptor.defaultMarker
              : BitmapDescriptor.defaultMarker,
          //  await MarkerIcon.widgetToIcon(globalKeymap[index.toString()]!) ,
          position: computeCentroid(
            coordinates: zoneList[index].formattedCoordinates!,
          ),
        ),
      );
    }
