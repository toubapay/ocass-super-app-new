...
class ServiceCartController extends GetxController implements GetxService {
  ...
  ServiceCartController({required this.cartRepo});
  ...
  Future<void> addToCartApi(
    ...
    if (providerId != "") {
      await cartRepo.addToCartListToServer(
        CartModelBody(
  ...
  void updatePreselectedProvider(
  ...
  void showMinimumAndMaximumOrderValueToaster() {
    ...
    if (configModel.content?.minBookingAmount != 0 &&
        (configModel.content?.minBookingAmount ?? 0) > _totalPrice &&
      ...
      customSnackBar(
        ...
        customWidget: Row(
          children: [
            ...
            Text(
              "  ${'minimum_booking_amount'.tr} ${PriceConverter.convertPrice(Get.find<ServiceSplashController>().configModel.content!.minBookingAmount!)}",
      ...
      if (configModel.content?.maxBookingAmount != 0 &&
          (configModel.content?.maxBookingAmount ?? 0) < _totalPrice &&
          _cartList.isNotEmpty) {
        customSnackBar(
          "message",
          customWidget: Column(
                  ...
                  Flexible(
                    child: Text(
                      ...
                      "(${'${'maximum_order_amount'.tr}'
                          ' ${PriceConverter.convertPrice(Get.find<ServiceSplashController>().configModel.content?.maxBookingAmount ?? 0)}'}) ${"admin_will_verify_this_order".tr}",
                      style: robotoRegular.copyWith(color: Colors.white),
                    ),
                  ),
                ],
  ...
  String maskNumberWithoutCountryCode(String phoneNumber) {
    ...
    String maskedNumber = phoneNumber.substring(
      0,
