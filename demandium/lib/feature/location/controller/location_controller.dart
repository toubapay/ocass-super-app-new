...
class ServiceLocationController extends GetxController implements GetxService {
  final LocationRepo locationRepo;
  ServiceLocationController({required this.locationRepo});
  ...
  Future<AddressModel> getCurrentLocation(
    ...
    print(
      '--------------res-----> ${responseModel.zoneIds} || ${getUserAddress()?.zoneId}',
    );
  ...
  Future<void> saveAddressAndNavigate(
    ...
    if (!fromAddressDialog &&
        ...
        : true && Get.find<ServiceCartController>().cartList.isNotEmpty) {
      Get.dialog(
        ConfirmationDialog(
          icon: Images.warning,
  ...
  void autoNavigate(
    ...
    if (canRoute && route != null && route != "" && route != "home") {
      ...
      Get.offAllNamed(
        ServiceRouteHelper.getMainRoute(
          'home',
    ...
    if (shouldCartDelete) {
      await Get.find<ServiceCartController>().removeAllCartItem();
    }
  }
