...
class AllSearchController extends GetxController implements GetxService {
  ...
  Future<void> populatedSearchController(
    String queryText, {
  ...
  Future<void> searchData({
    ...
    Response response = await searchRepo.getSearchData(
      ...
      sortByType: _selectedSortByType != "default" ? _selectedSortByType : "",
      minPrice: _filteredMinPrice ?? 0,
      maxPrice:
          _filteredMaxPrice ?? _serviceModel?.content?.initialMaxPrice ?? 0,
      rating: _selectedRating,
      categoryIdes: _selectedCategoryId,
    );
    if (response.statusCode == 200) {
      ...
      if (_searchServiceList != null && offset != 1) {
        _searchServiceList!.addAll(
          _serviceModel?.content?.servicesContent?.serviceList ?? [],
        );
      } else {
        _searchServiceList = [];
        _searchServiceList!.addAll(
  ...
  Future<void> getSearchSuggestion(
    String query, {
    bool shouldUpdate = true,
  }) async {
    Response response = await searchRepo.getSearchSuggestion(
      query: searchController.text,
    );
    if (response.statusCode == 200) {
      _searchSuggestionModel = SearchSuggestionModel.fromJson(response.body);
      if (_searchSuggestionModel != null &&
          _searchSuggestionModel!.searchSuggestionContent != null &&
          _searchSuggestionModel!.searchSuggestionContent!.isNotEmpty) {
        _searchSuggestionList = [];
        _searchSuggestionList =
            _searchSuggestionModel!.searchSuggestionContent!;
      }
    } else {
  ...
  void updateIsFavoriteValue(
    ...
    if (_searchServiceList != null) {
      int? index = _searchServiceList?.indexWhere(
  ...
  void insertCategoryCheckedList() {
    Get.find<ServiceCategoryController>().categoryList?.forEach((element) {
  ...
  void updatedIsSortedAppliedStatus({bool shouldUpdate = true}) {
    ...
    if (_selectedSortBy != null) {
      sortedByList.add(
        FilterModel(title: _selectedSortBy, type: AllFilterType.sortBy),
      );
    }
    if (_selectedSortByType != "default") {
      sortedByList.add(
        FilterModel(title: _selectedSortByType, type: AllFilterType.sortByType),
      );
    }
    if (searchController.text.isNotEmpty) {
      sortedByList.add(
        FilterModel(title: searchController.text, type: AllFilterType.query),
      );
    }
  ...
  void updatedIsFilteredAppliedStatus({bool shouldUpdate = true}) {
    ...
    if ((_filteredMinPrice != null &&
            _serviceModel?.content?.filteredMinPrice != _filteredMinPrice) ||
        (_filteredMaxPrice != null &&
            _serviceModel?.content?.filteredMaxPrice != _filteredMaxPrice)) {
      filteredByList.add(
        FilterModel(title: "price", type: AllFilterType.price),
      );
    }
    if (_selectedRating != null) {
      filteredByList.add(
        FilterModel(title: "rating", type: AllFilterType.rating),
      );
    }
    ...
    if (_selectedCategoryId.isNotEmpty) {
      for (var element in _selectedCategoryId) {
        filteredByList.add(
          FilterModel(title: "category", type: AllFilterType.category),
        );
