...
class CreatePostController extends GetxController implements GetxService {
  ...
  TextEditingController additionalInstructionController =
  ...
  Future<void> createCustomPost(
    ...
    AddressModel? addressModel =
        Get.find<ServiceLocationController>().selectedAddress;
    ...
    Response response = await createPostRepo.createCustomPost(
      CreatePostBody(
        ...
        addressId: addressModel?.id == "null" || addressModel?.id == null
            ? ""
            : addressModel?.id,
    ...
    if (response.statusCode == 200) {
      ...
      Get.offNamed(ServiceRouteHelper.getCreatePostSuccessfullyScreen());
      customSnackBar(
    ...
    if (response.statusCode == 200) {
      ...
      for (var data in postModel!.content!.data!) {
        if (!dateList.contains(
          DateConverter.dateStringMonthYear(DateTime.tryParse(data.createdAt!)),
        )) {
          dateList.add(
  ...
  Future<void> getProvidersOfferList(
    int offset,
    String postId, {
    reload = false,
  }) async {
    ...
    Response response = await createPostRepo.getProvidersOfferList(
      offset,
      postId,
    );
  ...
  Future<void> providerBidDetailsForNotification(
    String postId,
    String providerId,
  ) async {
    Response response = await createPostRepo.getProviderBidDetails(
      postId,
      providerId,
    );
    if (response.statusCode == 200) {
      if (response.body['content'] != null) {
        notificationBiddingDetails = ProviderOfferData.fromJson(
          response.body['content']['post_bid'],
        );
        ...
        showGeneralDialog(
          ...
          pageBuilder: (context, _, a) {
            return ProviderBiddingNotificationDialog(
              providerOfferData:
                  notificationBiddingDetails ?? ProviderOfferData(),
              postId: postId,
            );
          },
  ...
  Future<Response> updatePostStatus(
    String postId,
    String providerId,
    String status, {
    int? isPartial,
    ...
    Response response = await createPostRepo.updatePostStatus(
      postId,
      providerId,
      status,
      isPartial: isPartial,
  ...
  void selectCategory(String id) {
    if (Get.find<ServiceCategoryController>().categoryList != null) {
      for (var element in Get.find<ServiceCategoryController>().categoryList!) {
        if (element.id == id) {
