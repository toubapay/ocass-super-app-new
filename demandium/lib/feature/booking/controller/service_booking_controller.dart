...
class ServiceBookingController extends GetxController implements GetxService {
  ...
  Future<void> checkCartSubcategory(
    ...
    if (Get.find<ServiceCartController>().cartList.isNotEmpty) {
      List<CartModel> cartList = Get.find<ServiceCartController>().cartList;
      if (cartList[0].subCategoryId != subcategoryId) {
        Get.dialog(
          ConfirmationDialog(
            ...
            onYesPressed: () async {
              Get.find<ServiceCartController>().removeAllCartItem();
  ...
  List<PopupMenuModel> getPopupMenuList({
    ...
    if (status == "pending") {
      ...
      return [
        ...
        if (!isRepeatBooking && !isCustomizeBooking)
