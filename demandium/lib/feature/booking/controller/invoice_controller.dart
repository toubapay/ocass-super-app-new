...
class InvoiceController {
  static Future<Uint8List> generateUint8List(
    ...
    try {
      imageProvider = await downloadImage(
        Get.find<ServiceSplashController>().configModel.content!.logoFullPath ??
            "",
      );
    } catch (e) {
      if (kDebugMode) {
  ...
  static Widget invoiceIDSchedule(
    String invoiceID,
    ...
    children: [
      ...
      Text(
        "Service Schedule : ${DateConverter.dateMonthYearTimeTwentyFourFormat(DateTime.tryParse(invoiceSchedule ?? "")!)}",
      ),
    ],
  );
      ...
      Row(
        ...
        children: [
          serviceAddress(
            ...
            address:
                content.serviceAddress!.address?.replaceAll("null", "") ?? "",
  ...
  static Widget serviceAddress({
    ...
    children: [
      ...
      if (email != '') buildSimpleText(title: 'Email', value: email),
      if (phone != '') buildSimpleText(title: 'Phone', value: phone),
      if (address != '')
        SizedBox(width: Get.width * .4, child: Text('Address: $address')),
    ],
  );

  static Widget buildProviderSection(
    Provider provider,
    ServicemanInvoice serviceman,
  ) => Column(
  ...
  static Widget buildServiceManSection(Serviceman serviceman) =>
  ...
  static Widget buildInvoiceInfo(InvoiceInfo info) {
    ...
    final data = <String?>[info.number, info.date, info.paymentStatus];
                        ...
                        children: [
                          ...
                          ListView.builder(
                            itemBuilder: (context, index) {
                              ...
                              return buildText(
                                title:
                                    '${paidWith == "digital" && bookingDetailsContent.paymentMethod == "offline_payment"
                                        ? ""
                                        : (paidWith == "wallet" || paidWith == "digital")
                                        ? "Paid by '"
                                        : ""}'
                                    '${paidWith == "digital" && bookingDetailsContent.paymentMethod == "offline_payment"
                                        ? "Offline Payment"
                          ...
                          if (bookingDetailsContent.additionalCharge != null &&
                            ...
                            Column(
                              children: [
                                buildText(
                                  title:
                                      'Paid Amount (${bookingDetailsContent.paymentMethod == "offline_payment"
                                          ? "Offline"
                                          : bookingDetailsContent.paymentMethod == "wallet_payment"
                                          ? "Wallet"
                                          : bookingDetailsContent.paymentMethod == "cash_after_service"
                                          ? "Cash After Service"
                                          : bookingDetailsContent.paymentMethod})',
                                  titleStyle: TextStyle(
                                    fontSize: 13,
  ...
  static Row buildSimpleText({required String title, required String value}) {
