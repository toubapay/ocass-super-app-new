  ...
  Widget build(BuildContext context) {
    return GetBuilder<BookingDetailsController>(
      builder: (bookingDetailsController) {
        bool isEdited =
            bookingDetails.repeatEditHistory != null &&
            bookingDetails.repeatEditHistory!.isNotEmpty;
        ...
        double initialSubTotal =
            BookingHelper.getSubTotalCost(bookingDetails) *
            ((bookingDetails.totalCount ?? 1));
        ...
        double paidAmount = BookingHelper.getRepeatBookingPaidAmount(
          bookingDetails,
        );
        double canceledAmount = BookingHelper.getRepeatBookingCanceledAmount(
          bookingDetails,
        );
        int paidBookingCount = BookingHelper.getRepeatPaidBookingCount(
          bookingDetails,
        );
        int canceledBookingCount = BookingHelper.getRepeatCanceledBookingCount(
          bookingDetails,
        );
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _SubTotalItemWidget(
                      title: isEdited
                          ? "${'initial_sub_total'.tr}  (${bookingDetails.totalCount ?? ""} ${'days'.tr})"
                          : "${'sub_total'.tr}  (${bookingDetails.totalCount ?? ""} ${'days'.tr})",
                    ...
                    if (paidAmount > 0)
                      _SubTotalItemWidget(
                        title:
                            "${'paid'.tr} (${'for'.tr} $paidBookingCount ${'bookings'.tr})",
                        amount: paidAmount,
                        additionalSign: "",
                        subTitle: "",
                      ),
                    if (paidAmount > 0 || canceledAmount > 0)
                      const SizedBox(height: Dimensions.paddingSizeExtraSmall),

                    if (canceledAmount > 0)
                      _SubTotalItemWidget(
                        title:
                            "${'canceled_amount'.tr} ($canceledBookingCount ${'bookings'.tr})",
                        amount: canceledAmount,
                        additionalSign: "-",
                        subTitle: "",
                      ),
                    if (canceledBookingCount > 0)
                      const SizedBox(height: Dimensions.paddingSizeExtraSmall),
    ...
    return Column(
      ...
      children: [
        ...
        if (bookingService?.variantKey != null)
          Padding(
            padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeTine),
            child: Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    if (bookingDetails?.repeatEditHistory != null &&
                        bookingDetails!.repeatEditHistory!.isNotEmpty)
                      InkWell(
                        onTap: () {
                          showDialog(
                            context: context,
                            builder: (ctx) =>
                                const RepeatBookingEditHistoryDialog(),
                          );
                        },
                        child: Text(
                          '(${'updated'.tr})',
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
              ...
              if (subTitle != "")
                InkWell(
                  onTap: () {
                    showDialog(
                      context: context,
                      builder: (ctx) => const RepeatBookingEditHistoryDialog(),
                    );
                  },
                  child: Text(
                    '(${'view_details'.tr})',
        ...
        const SizedBox(width: Dimensions.paddingSizeDefault),
        Text(
          "${additionalSign != "" ? "($additionalSign)" : ""} ${PriceConverter.convertPrice(amount, isShowLongPrice: true)}",
          style: robotoRegular.copyWith(
            fontSize: Dimensions.fontSizeDefault,
            color: Theme.of(
              context,
