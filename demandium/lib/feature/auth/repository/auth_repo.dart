...
class AuthRepo {
    ...
    if (!GetPlatform.isWeb) {
      if (Get.find<ServiceLocationController>().getUserAddress() != null) {
        FirebaseMessaging.instance.subscribeToTopic(
          '${AppConstants.topic}-${Get.find<ServiceLocationController>().getUserAddress()!.zoneId!}',
        );
      }
  ...
  Future<void> subscribeTokenToTopic(dynamic token, dynamic topic) async {
    await apiClient.postData(AppConstants.subscribeToTopic, {
      "token": '$token',
      "topic": topic,
    });
  }
  ...
  Future<Response?> verifyOtpForPhoneOtpLogin({
    String? phone,
    required String otp,
  }) async {
  ...
  bool clearSharedData({Response? response}) {
    if (GetPlatform.isAndroid && !GetPlatform.isWeb) {
      if (Get.find<ServiceLocationController>().getUserAddress() != null) {
        if (Get.find<ServiceLocationController>().getUserAddress()!.zoneId !=
            null) {
    ...
    Get.find<ServiceAuthController>().updateSavedLocalAddress(
      saveContactPersonInfo: false,
    );
    ...
    if (response != null &&
      ...
      AddressModel? address = Get.find<ServiceLocationController>()
          .getUserAddress();
      address?.zoneId = "";
      Get.find<ServiceLocationController>().saveUserAddress(
  ...
  void toggleReferralBottomSheetShow(bool isShowBottomSheet) {
    sharedPreferences.setBool(
      AppConstants.referredBottomSheet,
      isShowBottomSheet,
    );
  }
