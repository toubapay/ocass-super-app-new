...
class ConfigContent {
  ...
  ConfigContent.fromJson(Map<String, dynamic> json) {
    ...
    providerSelfRegistration = int.tryParse(
      json['provider_self_registration'].toString(),
    );
    ...
    firebaseOtpVerification = int.tryParse(
      json['firebase_otp_verification'].toString(),
    );
    ...
    additionalChargeFeeAmount = double.tryParse(
      json['additional_charge_fee_amount'].toString(),
    );
    additionalCharge = int.tryParse(
      json['booking_additional_charge'].toString(),
    );
    ...
    if (json['confirm_otp_for_complete_service'] != null) {
      confirmationOtpStatus = json['confirm_otp_for_complete_service'] == 1
          ? true
          : false;
    }
    ...
    scheduleBookingTimeRestriction = int.tryParse(
      json['schedule_booking_time_restriction'].toString(),
    );
    ...
    createGuestUserAccount = int.tryParse(
      json['create_user_account_from_guest_info'].toString(),
    );
    serviceAtProviderLocation = int.tryParse(
      json['service_at_provider_place'].toString(),
    );
    newsletterTitle = json['newsletter_title'] != ""
        ? json['newsletter_title']
        : null;
    newsletterDescription = json['newsletter_description'] != ""
        ? json['newsletter_description']
        : null;
    businessPages = json["business_pages"] == null
        ? []
        : List<BusinessPage>.from(
            json["business_pages"]!.map((x) => BusinessPage.fromJson(x)),
          );
...
class MaintenanceMode {
  ...
  Map<String, dynamic> toJson() {
    ...
    if (maintenanceTypeAndDuration != null) {
      data['maintenance_type_and_duration'] = maintenanceTypeAndDuration!
          .toJson();
    }
    return data;
  }
...
class BusinessPage {
  ...
  BusinessPage({this.pageKey, this.isDefault, this.title});
