...
class PriceConverter {
  ...
  static String convertPrice(
    ...
    bool isRightSide =
        Get.find<ServiceSplashController>()
            ...
            'right' &&
        Get.find<ServiceLocalizationController>().isLtr == true;
    return isShowLongPrice == true
        ? '${isRightSide ? '' : getCurrency()}'
              '${(price!).toStringAsFixed(decimalPointCount ?? int.parse(Get.find<ServiceSplashController>().configModel.content?.currencyDecimalPoint ?? '0')).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
              '${isRightSide ? getCurrency() : ''}'
        : longToShortPrice(
            '${isRightSide ? '' : getCurrency()}'
            '${(price!).toStringAsFixed(decimalPointCount ?? int.parse(Get.find<ServiceSplashController>().configModel.content?.currencyDecimalPoint ?? '0')).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
  ...
  static Discount _getDiscount(
    List<ServiceDiscount>? serviceDiscountList,
  ...
  static Discount discountCalculation(
    Service service, {
